...
// https://github.com/ng-select/ng-select/blob/master/src/ng-select/lib/ng-dropdown-panel.service.ts
  ...
  for (var i = 0; i < countries.length; i++) {
    if (countries[i].toLowerCase().indexOf(val) > -1) {
      ...
      item.addEventListener('click', function(index) {
        return function(e) {
          inputEle.value = countries[index];
          closeAllLists();
        };
      }(i));
      item.addEventListener('mouseenter', function(index) {
        return function(e) {
          currentFocus = index;
          var x = listDiv.getElementsByTagName('div');
          addActive(x);
        };
      }(counter));
...
function handleKeyOperations(e) {
  var x = document.getElementById(inputEle.id + '-autocomplete-list');
  if (x) {
...
function closeAllLists(elemt) {
  ...
  for (var i = 0; i < list.length; i++) {
    if (elemt !== list[i] && elemt !== inputEle) {
